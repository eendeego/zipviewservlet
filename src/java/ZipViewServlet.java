 ...
 *
 * @version 2.0
 * @author  Luis Reis (lmfr@netscape.net)
 */
public class ZipViewServlet extends HttpServlet {
	...
	hash_mime_types = new Hashtable();
	for(int i=0;i<mime_types.length;i++)
	    hash_mime_types.put(mime_types[i][0],mime_types[i][1]);
    }
	...
	boolean browsing = session.getValue("browsemode")!=null;
	...
	if(query_string!=null && query_string.length()!=0) {
		...
		session.putValue("browsemode",new Boolean(true));
		res.sendRedirect(getFullServletPath(req).append('/').toString());
	    } else if("nobrowse".equals(query_string)) {
		session.removeValue("browsemode");
		res.sendRedirect(getFullServletPath(req).append('/').toString());
     ...
     */
    String getStyleSheet(HttpServletRequest req) {
	return "<LINK REL=STYLESHEET TYPE=\"text/css\" HREF=\"" +
	    req.getServletPath() + "/.resource/styles/style.css\">";
    }
	...
	boolean browsing = (req.getSession(true).getValue("browsemode") != null);
  ...
  public void cantOpenZip(String zip_alias,
    ...
    StringBuffer new_page=HttpUtils.getRequestURL(req);
